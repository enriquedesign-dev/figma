...
# Figma API Production Deployment Script
...
read -p "🤔 Install Nginx reverse proxy? This adds domain support and better security. (y/n): " install_nginx
...
EOF
    ...
    # Add rate limiting zone to main nginx config
    sudo sed -i '/http {/a\    limit_req_zone $binary_remote_addr zone=api:10m rate=100r/m;' /etc/nginx/nginx.conf
    ...
    echo "📋 Skipping Nginx setup - API available directly on port 8000"
    echo "🌐 Your API is available at: http://168.119.237.216:8000/"
fi
...
if [[ $install_nginx == "y" || $install_nginx == "Y" ]]; then
    ...
    echo "🌐 Your API endpoints are now available at:"
    echo "  http://168.119.237.216:8000/health"
    echo "  http://168.119.237.216:8000/api/figma/texts"
    echo "  http://168.119.237.216:8000/api/figma/pages"
fi

echo ""
echo "🔧 For mobile app integration, use these base URLs:"
if [[ $install_nginx == "y" || $install_nginx == "Y" ]]; then
    echo "  Production: http://168.119.237.216/api/"
else
    echo "  Production: http://168.119.237.216:8000/api/"
fi 